...
const dmChannelSchema = require('../database/Schemas/dmChannels');
...
const dm = require('../functions/dm/dm');

const dmParent = '794488848230252565'
...
const existingDM = new Set(); // existing dm set of non server members
    ...
    bot.on('message', async (message) => {
        ...
        if (message.channel.type === "dm") {
            ...
            if (message.channel.parentID === dmParent) {
                ...
                if (findDMPerson.length > 0) {
                    const [{ _id }] = findDMPerson

                    const dmPerson = await bot.users.fetch(_id);
                    if (dmPerson) {
                        dmPerson.send(message.content || '`file`');
                        if (message.attachments) {
                            message.attachments.forEach((attachment) => {
                                dmPerson.send(attachment.url);
                            })
                        }
                    }
                } else {
